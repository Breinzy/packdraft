-- Resumable market jobs. Catalog import and daily price sync cannot finish inside
-- one Vercel Hobby invocation (maxDuration 300s) or one PokemonPriceTracker
-- credit window, so each run persists a cursor and continues later.
