...
export async function POST(request: Request) {
  ...
  if (!tournamentId || !assetId) {
    return NextResponse.json({ error: 'tournamentId and assetId are required' }, { status: 400 });
  }
  ...
  try {
    ...
    const { data, error } = await service.rpc('execute_tournament_trade', {
