...
export default function AssetCard({ asset, href }: AssetCardProps) {
  ...
  const change = asset.change_7d ?? null;

  return (
    <Link
      href={href ?? `/assets/${asset.id}`}
      className="panel panel-hover flex min-h-[6.25rem] gap-4"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-sm font-semibold text-foreground md:text-[0.95rem]">{asset.name}</div>
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          <span className="num text-sm font-medium text-foreground">
            {asset.price == null ? '—' : formatCurrency(asset.price)}
          </span>
          {asset.price != null && change != null ? <Delta pct={change} /> : null}
          {asset.price == null ? (
